...
if (document.querySelector('.activities__body')) {
    ...
    const toStart = nav.querySelector('.slider-arrow_next2');
    const toEnd = nav.querySelector('.slider-arrow_prev2');
    let activitiesSlider = new Swiper('.activities__body', {
        ...
        on: {
            ...
            slideChange: function () {
                if (nav.querySelector('.slider-arrow_next.swiper-button-disabled')) {
                    toStart.classList.remove('_hide');
                } else {
                    toStart.classList.add('_hide');
                }
                if (nav.querySelector('.slider-arrow_prev.swiper-button-disabled')) {
                    toEnd.classList.remove('_hide');
                } else {
                    toEnd.classList.add('_hide');
                }
            },
        }
    });
    document.addEventListener("click", function (e) {
        const target = e.target
        if (target.closest(".slider-arrow_next2")) {
            activitiesSlider.slideTo(0);
        }
        // if (target.closest(".slider-arrow_prev2")) {
        //     activitiesSlider.slideTo(activitiesSlider.slides.length - 1);
        // }
    });
    window.addEventListener("load", function () {
...
if (document.querySelector('.activities2__body')) {
    const activity = document.querySelector('.activities2__slider');
    const nav = activity.querySelector('.activities2__nav');
    let activitiesSlider = new Swiper('.activities2__body', {
        observer: true,
        observeParents: true,
        speed: 800,
        watchOverflow: true,
        ...
        // Dotts
        pagination: {
            el: '.activities2__pagination',
            type: 'fraction',
        },
        // Arrows
        navigation: {
            nextEl: '.activities2__nav .slider-arrow_next',
            prevEl: '.activities2__nav .slider-arrow_prev',
        },
        on: {
            slideChangeTransitionStart: function () {
                nav.classList.add('_none')
            ...
            /* slideChange: function () {
                if ((activitiesSlider.realIndex + 1) == +(activity.dataset.slides)) {
                    activitiesSlider.slides[activitiesSlider.realIndex].classList.add('_ready');
                } else {
                    const slideReady = activity.querySelector('._ready');
                    if (slideReady) {
                        slideReady.classList.remove('_ready');
                    }
                }
            }, */
            // init: function () {
    ...
    // const slides = activity.querySelectorAll('.activities2__slide');
    // document.addEventListener("DOMContentLoaded", function () {
    //     activity.dataset.slides = slides.length;
    // });
    window.addEventListener("load", function () {
        activitiesSlider.init();
    });
